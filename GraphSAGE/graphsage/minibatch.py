...
class WeightedNodeMinibatchIterator(NodeMinibatchIterator):
    ...
    def __init__(self, CModel,centroids, dif_weight = 1, directed_graph = False, same_weight = 1, **kwargs):
    ...
    def construct_adj(self):
        ...
        distance = {}
        for id1, i in enumerate(self.centroids):
            for id2, j in enumerate(self.centroids):
                distance[(id1 - 1, id2 - 1)] = np.linalg.norm(i - j)
        ...
        if self.directed_graph:
            ...
            for nodeid in self.G.nodes():
                ...
                for i, nid in enumerate(neighbors):
                    #mask[self.id2idx[nodeid], i] = self.pweight if self.node_tags[nid] == node_tag else self.nweight
                    mask[self.id2idx[nodeid], i] = distance[(self.node_tags[nid],node_tag)]
